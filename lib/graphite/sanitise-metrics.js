...
const sanitiseMetrics = ({ metrics_prefix, metrics }) => {
	...
	let valid_metrics = _.map(noNulls, (value, key) => {
		...
		// Graphite metrics can only be an integer or decimal. Set value for the metrics to zero if value is not a number
		if (typeof value !== 'number') {
			value = 0;
			metrics_error = true;
		}
		// Graphite metrics name can only be 255 characters long. 
		// Shorten the metrics name as a keyerror if metrics name is too long
		// Reserving 55 characters for prefix
		if (key.length > 200) {
	...
	// Log the error in metrics to splunk for alerting teams
	if (metrics_error) {
