...
Metrics.prototype.init = function (opts) {
	...
	const disableGraphiteMetrics = (FT_GRAPHITE_UUID && FT_GRAPHITE_UUID === 'false');
	...
	if (!disableGraphiteMetrics && isProduction && noValidGraphiteUuid) {
		throw new Error('next-metrics: The environment variable FT_GRAPHITE_UUID must be explicitly set to \'false\' if you don\'t wish to send metrics to FT\'s internal Graphite');
	}
	...
	if (disableGraphiteMetrics) {
		logger.info('next-metrics: FT_GRAPHITE_UUID is set to \'false\', metrics will not be sent to FT\'s internal Graphite');
	}
