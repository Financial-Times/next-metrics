...
const FT_GRAPHITE_HOSTNAME = 'graphite.ft.com';
...
Metrics.prototype.init = function (opts) {
	...
	const FT_GRAPHITE_APIKEY = process.env.FT_GRAPHITE_APIKEY;
	...
	if (!disableGraphiteMetrics && isProduction && noValidGraphiteApiKey) {
		hasValidConfiguration = false;
		logger.error({ event: 'NEXT_METRICS_INVALID_PRODUCTION_CONFIG', message: 'The environment variable FT_GRAPHITE_APIKEY must be explicitly set to \'false\' if you don\'t wish to send metrics to FT\'s internal Graphite' });
	}
	if (disableGraphiteMetrics) {
		logger.info({ event: 'NEXT_METRICS_DISABLED', message: 'FT_GRAPHITE_APIKEY is set to \'false\', metrics will not be sent to FT\'s internal Graphite' });
	}
