...
const { Proxies, ExpressProxies, ConnectProxies } = require('./metrics/index.js');
...
Metrics.prototype.init = function (opts) {
	...
	if(this.opts.useDefaultAggregators !== false && this.opts.isRouter !== true){
		this.httpReq = new ExpressProxies.HttpRequest();
		this.httpRes = new ExpressProxies.HttpResponse();
		this.system = new Proxies.System();
		this.setupDefaultAggregators();
	} else if (this.opts.isRouter) { 
		this.httpReq		= new ConnectProxies.HttpRequest();
		this.httpRes		= new ConnectProxies.HttpResponse();
		this.proxyServer	= new ConnectProxies.ProxyServer();
		this.httpServer		= new ConnectProxies.HttpServer();
		this.system			= new Proxies.System();
		this.setupDefaultAggregators();
	}
...
Metrics.prototype.setupDefaultAggregators = function () {
	...
	if (this.opts.isRouter) {
		this.registerAggregator(this.proxyServer.reporter);
		this.registerAggregator(this.httpServer.reporter);
	}
};
...
Metrics.prototype.instrument = function (obj, opts) {
	...
	switch (opts.as) {
		...
		case 'http.proxy':
			...
			break;
		case 'http.server':
			this.httpServer.instrument(obj);
			break;
		case 'system':
