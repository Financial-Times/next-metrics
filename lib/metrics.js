...
Metrics.prototype.init = function (opts) {
	...
	let FT_GRAPHITE_APP_UUID = process.env.FT_GRAPHITE_APP_UUID;

	const isProduction = (process.env.NODE_ENV === 'production');
	const disableGraphiteMetrics = (FT_GRAPHITE_APP_UUID && FT_GRAPHITE_APP_UUID === 'false');
	const noValidGraphiteUuid = (!FT_GRAPHITE_APP_UUID);
	...
	if (!disableGraphiteMetrics && isProduction && noValidGraphiteUuid) {
		throw new Error('next-metrics: The environment variable FT_GRAPHITE_APP_UUID must be explicitly set to \'false\' if you don\'t wish to send metrics to FT\'s internal Graphite');
	}
	if (disableGraphiteMetrics) {
		logger.info('next-metrics: FT_GRAPHITE_APP_UUID is set to \'false\', metrics will not be sent to FT\'s internal Graphite');
	}
