...
var Metrics = function (opts) {
    ...
    this.instrumentMap = {
        'httpReq' : ['express.http.req'],
        'httpRes' : ['express.http.res'],
        'ftApi' : ['ft-api-client']
    }
...
Metrics.prototype.instrument = function (obj, opts) {
    ...
    var method;
    var keys = Object.keys(this.instrumentMap);
    for(var i=0, l=keys.length; i<l; i++){
        if(this.instrumentMap[keys[i]].indexOf(opts.as) > -1){
            method = this[keys[i]].instrument.bind(this[keys[i]], obj, opts);
            break;
        }
    }

    if(!method){
        throw new Error(opts.as + 'is not a valid option for opts.as');
    }
