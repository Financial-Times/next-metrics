...
var ftApiKey = process.env.GRAPHITE_APIKEY;

if (!ftApiKey && process.env.NODE_ENV === 'production') throw "No GRAPHITE_APIKEY is set. Please set a false one if you are on your localhost.";
...
Metrics.prototype.init = function(opts) {
	...
	// If API keys have been explicitly set to `false`, we don't send metrics
	var destinations = [];
