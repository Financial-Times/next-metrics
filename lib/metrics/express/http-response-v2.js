...
HttpResponse.prototype.counts = function() {
	...
	Object.keys(this.buckets).forEach(function (key) {
		var safeKey = key.indexOf('template' > -1) ? key.replace(/[\/\\\.]/g, '_') : key;
		metrics[safeKey + '.count'] = this.buckets[key].counter.count;
		metrics[safeKey + '.time.sum'] = this.buckets[key].timer.sum;
		metrics[safeKey + '.time.max'] = this.buckets[key].timer.max;
		metrics[safeKey + '.time.min'] = this.buckets[key].timer.min;
		metrics[safeKey + '.time.mean'] = this.buckets[key].timer.mean;
		metrics[safeKey + '.time.stdDev'] = this.buckets[key].timer.stdDev;
	}.bind(this));
...
HttpResponse.prototype._writeHead = function(fn, dt) {
	...
	return scarlet
		...
		.using(function(invocation, proceed) {
			...
			self.measure('status_' + statusCode, Date.now() - dt);
			proceed();
		}).proxy();
};
